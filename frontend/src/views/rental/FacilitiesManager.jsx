...
import { getFacilities, createFacility, updateFacility, deleteFacility } from '../../services/rentalApi';
...
const FacilitiesManager = () => {
  ...
  const loadRows = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getFacilities({ page, limit: 10, search: search.trim() || undefined });
      setRows(res.data?.data ?? []);
      const p = res.data?.pagination ?? {};
      setPagination({ totalPages: p.totalPages ?? 1, total: p.total ?? 0 });
    } catch (e) {
      console.error(e);
      setMessage({ error: 'Failed to load facilities.', success: '' });
    } finally {
      setLoading(false);
    }
  }, [page, search]);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing?.id) {
        ...
        setMessage({ success: 'Facility updated.', error: '' });
      } else {
        await createFacility(payload);
        setMessage({ success: 'Facility created.', error: '' });
      }
      setShowModal(false);
      loadRows();
    } catch (e) {
  ...
  const remove = async (id) => {
    ...
    try {
      ...
      setMessage({ success: 'Facility deleted.', error: '' });
      loadRows();
    } catch (e) {
      console.error(e);
