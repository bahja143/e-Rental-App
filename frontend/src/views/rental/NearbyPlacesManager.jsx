...
import { getNearbyPlaces, createNearbyPlace, updateNearbyPlace, deleteNearbyPlace } from '../../services/rentalApi';

const emptyForm = { name_en: '', name_so: '' };

const NearbyPlacesManager = () => {
  ...
  const loadRows = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getNearbyPlaces({ page, limit: 10, search: search.trim() || undefined });
      setRows(res.data?.data ?? []);
      const p = res.data?.pagination ?? {};
      setPagination({ totalPages: p.totalPages ?? 1, total: p.total ?? 0 });
    } catch (e) {
      console.error(e);
      setMessage({ error: 'Failed to load nearby places.', success: '' });
    } finally {
      setLoading(false);
    }
  }, [page, search]);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing?.id) {
        await updateNearbyPlace(editing.id, payload);
        setMessage({ success: 'Nearby place updated.', error: '' });
      } else {
        await createNearbyPlace(payload);
        setMessage({ success: 'Nearby place created.', error: '' });
      }
      setShowModal(false);
      loadRows();
    } catch (e) {
  ...
  const remove = async (id) => {
    if (!window.confirm('Delete this nearby place?')) return;
    try {
      await deleteNearbyPlace(id);
      setMessage({ success: 'Nearby place deleted.', error: '' });
      loadRows();
    } catch (e) {
      console.error(e);
    ...
    <div className="rental-page">
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered className="rental-modal">
        <Modal.Header closeButton className="rental-modal-header">
          <Modal.Title>
            <i className="feather icon-map me-2" />
            {editing ? 'Edit Nearby Place' : 'Create Nearby Place'}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Row className="g-3">
...
export default NearbyPlacesManager;
